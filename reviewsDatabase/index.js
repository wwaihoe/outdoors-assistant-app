...
app.post('/reviews', async (req, res) => {
    try {
        ...
        const newReview = await pool.query(
            "INSERT INTO review (username, location_name, rating, description) VALUES($1, $2, $3, $4) RETURNING *",
...
app.get('/reviews/:location_name', async (req, res) => {
    try {
        ...
        const ReviewsForLocation = await pool.query("SELECT * FROM review WHERE location_name = $1", [location_name]);
...
app.get('/reviews/:location_name/average', async (req, res) => {
    try {
        ...
        const avgRating = await pool.query("SELECT AVG(rating) as average_rating FROM review WHERE location_name = $1", [location_name]);
...
app.get('/reviews/:username', async (req, res) => {
    try {
        ...
        const review = await pool.query("SELECT * FROM review WHERE username = $1", [username]);
...
app.put('/reviews/:username/:location_name', async (req, res) => {
    try {
        ...
        const updateReview = await pool.query(
            "UPDATE review SET rating = $1, description = $2 WHERE username = $3 AND location_name = $4",
...
app.delete('/reviews/:username/:location_name', async (req, res) => {
    try{
        ...
        const deleteReview = await pool.query("DELETE FROM review WHERE username = $1 AND location_name = $2", [user_id, location_name]);
